...
namespace sdr
{
        ...
        : Node("sdr_keyboard_commander", options)
    {
        ...
        while (!change_state_client_->wait_for_service(std::chrono::seconds(1)))
        {
            ...
            RCLCPP_INFO(get_logger(), "Service '/%s/change_state' not available, waiting...", target_node_name_.c_str());
        }
    ...
    void SDRKeyboardCommander::request_transition(uint8_t transition_id)
    {
        ...
        {
            RCLCPP_ERROR(get_logger(), "Service '%s' is not available.", change_state_client_->get_service_name());
            return;
        }
    ...
    void SDRKeyboardCommander::get_current_state()
    {
        ...
        {
            RCLCPP_ERROR(get_logger(), "Service '%s' is not available.", get_state_client_->get_service_name());
            return;
        }
