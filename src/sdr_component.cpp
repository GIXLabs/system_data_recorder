...
void SystemDataRecorder::subscribe_to_topic(const std::string & topic, const std::string & type)
{
  ...
  auto offered_qos = get_offered_qos_for_topic(topic);
  ...
  // Jazzy uses named fields in TopicMetadata instead of brace-initialization
  ...
  topic_metadata.offered_qos_profiles = offered_qos;
...
std::vector<rclcpp::QoS> SystemDataRecorder::get_offered_qos_for_topic(const std::string & topic)
{
...
void SystemDataRecorder::unsubscribe_from_topics()
{
  ...
  for (const auto & topic_with_type : topics_and_types_) {
    ...
    meta.name = topic_with_type.first;
    meta.type = topic_with_type.second;
